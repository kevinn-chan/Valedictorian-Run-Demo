...
export function chain(ids: string[], rescue = true) {
  const rest: { id: string; make: () => LanguageModelV4 }[] = ids
    .slice(1)
    .map((id) => ({ id, make: () => google(id) }));
  if (rescue && process.env.OPENAI_API_KEY)
    rest.push({ id: RESCUE, make: () => openai(RESCUE) });

  // Which model actually answered is otherwise invisible: a compile or a card
  // deck generated by the last rung looks identical to one from the first, and
  // the models differ enough in density and instruction-following to matter.
  // Only fallbacks are logged — the happy path would be pure noise.
  const next = async <T>(
    call: (m: LanguageModelV4) => Promise<T>,
    first: unknown
  ): Promise<T> => {
    for (const { id, make } of rest) {
      try {
        const result = await call(make());
        console.warn(`[llm] ${ids[0]} unavailable — served by ${id}`);
        return result;
      } catch (e) {
        if (!isBusy(e)) throw e;
      }
    }
    console.error(`[llm] every model in the chain refused (head ${ids[0]})`);
    throw first;
  };
